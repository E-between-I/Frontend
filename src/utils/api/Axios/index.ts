...
interface UserRegistrationDto {
          ...
          if (err.response) {
            switch (err.response.status) {
              case 400:
                ...
                break;
              default:
                toast.error("포스트 작성에 실패했습니다. 개발자에게 문의해주세요.");
            }
          } else {
            toast.error("네트워크 연결 상태를 확인해주세요.");
          }
  ...
  export const usePostRealty = (postData: PostRealtytDto) => {
    const navigate = useNavigate();
  
    return useMutation(
      async () =>
        axios.post<void>(
          `${process.env.VITE_BASE_URL}${path}/post/realty`,
          postData
        ),
      {
        onSuccess: () => {
          toast.success("부당산 포스트 작성이 완료되었습니다.");
          navigate("/deposit")
        },
        onError: (err: AxiosError<AxiosError>) => {
          if (err.response) {
            switch (err.response.status) {
              case 400:
                ...
                break;
              default:
                toast.error("부동산 포스트 작성에 실패했습니다. 개발자에게 문의해주세요.");
            }
          } else {
            toast.error("네트워크 연결 상태를 확인해주세요.");
          }
          ...
          if (err.response) {
            switch (err.response.status) {
              case 400:
                ...
                break;
              default:
                toast.error("적금/예금 포스트 작성에 실패했습니다. 개발자에게 문의해주세요.");
            }
          } else {
            toast.error("네트워크 연결 상태를 확인해주세요.");
          }
          ...
          if (err.response) {
            switch (err.response.status) {
              case 400:
                ...
                break;
              default:
                toast.error("경제 포스트 작성에 실패했습니다. 개발자에게 문의해주세요.");
            }
          } else {
            toast.error("네트워크 연결 상태를 확인해주세요.");
          }
